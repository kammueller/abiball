...
// Abarbeiten
	...
	if ( ($erklaerung == "Hier kannst Du weitere Kommentare hinterlassen") OR ($erklaerung == "") ) { 
		//Wenn Keine Erklärung angegben worden ist
		$erklaerung = "keine Erklaerung"; }
	// Die erste Karte muss für den Abiturienten sein
	$result = mysqli_query($db_link, "SELECT * FROM `abi_user` WHERE `id` = '$user_id'");
	$datensatz = mysqli_fetch_array($result, MYSQL_ASSOC);
	$VorN = $datensatz['Vorname']; $NachN = $datensatz['Nachname'];
	if ( ($VorN1 != $VorN) OR ($NachN1 != $NachN) ) {
		include('../back-end/design_alpha.inc.php');
		include ('../back-end/design_beta.inc.php');
		echo "Du musst eine Karte (die erste) für dich selber bestellen.<br>
...
// Je nach Anzahl Karten
	switch ($wunsch) {
		...
		// Das selbe wie bei 4; Ausgeben, dass bloß 4 Karten, etc
			...
			$kosten = 4;
            $nachricht =  encode($bausteine[15])."<br>". encode($bausteine[16]);
        }
	}
